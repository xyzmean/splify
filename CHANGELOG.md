...
All notable changes to splify are documented here. Versions follow the package
`VERSION` file; the UCI config schema only ever changes **additively** (no renames
or removals), so upgrades never break an existing `/etc/config/splify`.
...
- **LuCI dashboard split into two tabs** — `Status` (diagnostics) and `Settings`
  (the UCI form), under `admin/services/splify`.
- **Live per-tunnel traffic** — `splify-doctor` reports cumulative `rx`/`tx`
...
- **Failover timeline** — transitions (`switch`, `recover`, `restart`,
  ...
  buffer (`/var/run/splify-events`) and rendered as a timeline.
  `splify-doctor --events` emits them as JSON.
- **Firewall auto-fix** — new `splify-firewall {check|fix} <iface>` creates or
...
- **rpcd/ubus object `splify`** (`status`/`events`/`action`) — the LuCI app now
...
- **Config migration** — `/etc/uci-defaults/99-splify-migrate` stamps `type=wg`
  on existing endpoints (additive, idempotent).

### Changed
- **Least-privilege ACL** — `luci-app-splify` now grants only the three
  `splify` ubus methods (read = `status`/`events`, write = `action`); the broad
