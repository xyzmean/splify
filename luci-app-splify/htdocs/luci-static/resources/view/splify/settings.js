...
// splify — форма расширенных настроек (вкладка «Дополнительно»). Простое
// управление — на вкладке «Главная» (view/splify/home.js); живая диагностика —
// в «Подробном состоянии». Здесь только UCI-форма.
...
return view.extend({
	...
	render: function (data) {
		...
		m = new form.Map('splify', _('Дополнительные настройки'),
			_('Тонкая настройка splify: режим маршрутизации, туннели с приоритетом, ' +
		...
		o = s.taboption('general', form.Value, 'interval', _('Интервал проверки (сек)'),
			_('Секунды между проверками туннелей и переключением при сбое.'));
		o.datatype = 'uinteger';
		o.placeholder = '180';

		o = s.taboption('general', form.Flag, 'killswitch', _('Блокировать трафик, если туннель недоступен'),
			_('Когда все туннели недоступны — блокировать VPN-трафик, а не выпускать его в WAN.'));
		...
		o = s.taboption('general', form.DynamicList, 'health_target', _('Адреса для проверки туннеля (ping)'),
			_('Пингуются через каждый туннель для проверки (любой ответ = туннель жив).'));
		o.datatype = 'ipaddr';
		o = s.taboption('general', form.Value, 'health_url', _('URL проверки WAN'),
			_('Запрашивается через WAN, чтобы выбрать между обходом DPI и прямым WAN.'));
		...
		o = s.taboption('lists', form.Value, 'vpn_domains_url', _('URL списка доменов для VPN'),
			_('Скачиваемые домены направляются в VPN на этапе DNS-разрешения (dnsmasq).'));
		o = s.taboption('lists', form.Value, 'ignore_domains_url', _('URL списка доменов напрямую'));

		o = s.taboption('lists', form.Flag, 'zapret_enabled', _('Использовать обход DPI (zapret)'),
			_('Пропускается автоматически, если zapret не установлен.'));
		o.default = '1';

		o = s.taboption('manual', form.DynamicList, 'vpn_cidr', _('Подсети для VPN'),
			_('Удалённые подсети, которые должны идти через туннель — обычно LAN ' +
		...
		o.datatype = 'cidr4';
		o = s.taboption('manual', form.DynamicList, 'direct_cidr', _('Подсети напрямую'),
			_('Подсети, которые держатся вне туннеля (всегда через WAN).'));
		o.datatype = 'cidr4';
		...
		// ---- failover tunnels ----
		s = m.section(form.GridSection, 'endpoint', _('Туннели (с приоритетом)'),
			_('Меньший номер приоритета — выше. Выбирайте интерфейсы, созданные в ' +
			  'Сеть → Интерфейсы.'));
		s.addremove = true;
		s.anonymous = true;
		s.sortable = true;
		...
		// Transport type seam (design §2.2): только `wg` реализован. Будущий
		// sing-box (vless/hysteria) бэкенд добавит сюда варианты. Поле с
		...
		s = m.section(form.GridSection, 'device', _('Правила для отдельных устройств'),
			_('Привяжите устройство LAN к VPN или к прямому каналу по его IP.'));
		s.addremove = true;
		s.anonymous = true;
