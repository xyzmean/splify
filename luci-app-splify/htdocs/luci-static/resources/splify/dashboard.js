...
// splify 2.0 dashboard. Pure status/diagnostics view — settings live on the
// sibling "Settings" tab (view/splify/settings.js). Talks to the splify rpcd
...
var callAction = rpc.declare({
	object: 'splify', method: 'action',
	params: [ 'name', 'iface' ]
});

// Severity -> CSS color (matches splify-doctor's OK/WARN/FIXABLE/FAIL).
var SEV = {
	OK:      '#3c763d',
	WARN:    '#8a6d3b',
	FIXABLE: '#a0522d',
...
function statusPanel(wgIfaces) {
	...
	function firstRun(d) {
		...
		if (!wgIfaces || !wgIfaces.length) {
			steps.push(E('li', {}, [
				...
				_(' (splify does not manage keys/peers).')
			]));
		} else {
		...
		steps.push(E('li', {}, [
			_('Add it on the '),
			E('a', { 'href': L.url('admin/services/splify/settings') }, _('Settings')),
	...
	// A "Fix automatically" button on firewall findings — runs splify-firewall
	...
	function fixButton(c) {
		...
		// Some firewall findings are NOT auto-fixable: a shared WAN/LAN zone, or an
			// iface already covered by a device-wildcard zone — splify-firewall refuses
		...
		return Promise.all([
			...
			if (!d || !d.summary) {
				body.replaceChildren(E('em', { 'style': 'color:' + SEV.FAIL },
					_('Diagnostics unavailable (is the splify service installed and running?)')));
				return;
			}
			render(d, events);
		}).catch(function (e) {
...
return baseclass.extend({
	...
	// statusPanel(wgIfaces) returns the full live dashboard node (own polling).
