...
`splify-doctor` называет каждую из этих проблем напрямую; исправление — в его
...
|---|---|
| `no failover tunnels configured` | Добавьте туннель в Services → splify (Туннели отказоустойчивости). |
| `LAN subnet not set or not detected` | Задайте подсеть/интерфейс LAN; до этого весь трафик идёт через WAN. |
| `<if>: interface does not exist` | Создайте WG/AWG-интерфейс в Network → Interfaces или уберите его из splify. |
...
| `<addr> not routed into tunnel` (site-to-site) | Подсеть пира в `vpn_cidr`, но не маршрутизируется — запустите `splify-apply` (на хабе подсеть спицы должна быть делегирована). |
| `<if>: route_allowed_ips not 0` | `splify-apply` — он принудительно ставит `route_allowed_ips=0`, чтобы туннель не перехватывал маршруты main. |
| `<if>: health probe failed` | Проверьте endpoint/ключи пира; failover пока выберет другой туннель. |
| `ipsum/ru set has N (<min)` | `splify-update-ipsum` / `-ru`; также перезаливается на след. тике. |
| `… list is stale` / `not yet downloaded` | Проверьте URL списка; запустите нужный `splify-update-*`. |
| `zapret is installed but not running` | Цикл failover его запустит; или `/etc/init.d/zapret start`. |
| `zapret is enabled … but not installed` | Установите zapret или снимите галку в Services → splify. |
| `dnsmasq nftset drop-in is missing` | `splify-apply` — регенерирует дроп-ин и перезагружает dnsmasq. |
...
> splify управляет **маршрутизацией**, но **не трогает межсетевой экран**.
...
lan↔туннель↔wan). Из CLI: `splify-firewall fix <iface>` (проверить, не меняя:
`splify-firewall check <iface>`). Рядом — прямая ссылка **«Настройки
межсетевого экрана»**.

## Аварийные команды

```sh
splify-disable     # немедленно: весь LAN в WAN (служба может вернуть обратно)
splify-doctor      # полный отчёт
splify-status      # компактный снимок рантайма
logread -e splify  # журнал службы/failover
splify-apply       # регенерировать nft/dnsmasq из UCI и переустановить маршруты
```

## Диагностический чек-лист

1. `splify-doctor` — посмотреть `overall` и FAIL/FIXABLE.
