...
# One-switch preset: a curated list of domains that geo-block Russian addresses,
# routed through the tunnel. It is a SEPARATE option from vpn_domains_url so that
# ticking the box in the dashboard never overwrites a URL the operator chose, and
# so both can be used at once (the updater merges them).
config_get_bool   GEOBLOCK_ENABLED   global geoblock_enabled   0
config_get        GEOBLOCK_IFACE     global geoblock_iface     ''
config_get        GEOBLOCK_URL       global geoblock_url \
...
# A second policy destination: domains from the geoblock list are routed through an
# interface the operator NAMES, not through whichever tunnel failover picked. It
# needs its own mark, table and rule because that is the whole point — the choice
# has to survive failover moving the main path elsewhere.
# 202 is free (200 = main policy path, 201 = the isolated health-probe table).
GEO_TABLE="202"
GEO_MARK="0x80000";       GEO_MASK="0x80000";       GEO_RULE_PRIO="997"
GEO_SET="splify_geo_v4"
...
SPLIFY_DNS_GEO_RULES="$SPLIFY_DNS_DIR/dns-geo-rules.lst"
...
DOMAINS_GEO_FILE="/etc/splify/geoblock-domains.lst"
